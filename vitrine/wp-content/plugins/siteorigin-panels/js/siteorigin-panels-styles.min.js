...
frame.open();});$s.find('.remove-image').click(function(){$s.find('.current-image').css('background-image','none');$s.find('input').val('');});});this.$('.style-field-measurement').each(function(){var $$=$(this);var text=$$.find('input[type="text"]');var unit=$$.find('select');var hidden=$$.find('input[type="hidden"]');if(hidden.val()!==''){var re=/([0-9\.,]+)(.*)/;var match=re.exec(hidden.val());if(match!=null&&typeof match[1]!=='undefined'&&typeof match[2]!=='undefined'){text.val(match[1]);unit.val(match[2]);}}
var setVal=function(){hidden.val(text.val()+unit.val());};text.keyup(setVal).change(setVal);unit.change(setVal);});}});})(jQuery,_,soPanelsOptions);